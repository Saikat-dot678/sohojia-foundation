...
router.post('/', upload, async (req, res, next) => {
    ...
    try {
        ...
        const {
            director_id, name, email, phone, password,confirm_password, aadhar_number,
            address, education, reimbursement, bank_acc_no, bank_ifsc
        } = req.body;
        if (password !== confirm_password) {
            return res.status(400).send('Passwords do not match.');
        }
        const dirId = parseInt(director_id, 10);
        const reimbursementValue = parseFloat(reimbursement);

        if (isNaN(dirId) || !name || !email || !phone || !password || !aadhar_number || !address || !education || isNaN(reimbursementValue) || !bank_acc_no || !bank_ifsc) {
            return res.status(400).send('All fields are required and must be valid.');
        }

        const [directorRows] = await pool.query('SELECT * FROM center_program_directors WHERE director_id = ?', [dirId]);

        if (!directorRows.length) {
            return res.redirect('/?error=Unauthorized registration attempt');
        }
        
        const directorData = directorRows[0];
        if (directorData.password_hash) {
            return res.redirect('/?warning=' + encodeURIComponent('This Director ID has already been registered. Please log in.'));
        }

        const ext = path.extname(req.file.originalname);
        gcsFileName = `directors/dir-${dirId}-${Date.now()}${ext}`;
        ...
        const updateSql = `
            UPDATE center_program_directors SET
