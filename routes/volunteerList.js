// backend/routes/volunteerlist.js
const express = require('express');
const router  = express.Router();
const pool    = require('../config/db'); // your mysql2 promise pool

// GET /volunteers?foundation_id=123
router.get('/', async (req, res, next) => {
  ...
  try {
    ...
    // 2) Fetch volunteers for that foundation
    const [vRows] = await pool.query(
      `SELECT volunteer_id, name, email
         FROM volunteers
        WHERE foundation_id = ?
        ORDER BY name`,
      [fid]
    );
    foundation.volunteers = vRows;

    // 3) Render the template
    res.render('Volunteers-list', { foundation });
  } catch (err) {
    next(err);
  }
});
